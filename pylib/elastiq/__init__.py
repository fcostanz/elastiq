...
#             -- modified by Francesco Costanza <francesco.costanza@ba.infn.it> 
#
#
...
# shutting down VMs via its OpenStack API interface using pycurl library.
#
...
cf['api'] = {
  # Configuration to access OpenStack API
  'server_url': 'https://dummy.keystone.server/keystone/',
  'username': 'my_username',
  'password': 'my_password',
  'tenant_name': 'my_tenant',
  'renew_token_every_s':  1200,
...
def gethostbyceleryname(name):
   """Returns the IPv4 address of a host given its Celery name."""
   consumer = None
   channel = None
   ip = None   
...
def gethostbyname(name):
  ...
  if batch_name == 'htcondor':
    return gethostbycondorname(name)
  elif batch_name == 'eq4celery':
    return gethostbyceleryname(name)
...
def scale_up(nvms, valid_hostnames=None, vms_allegedly_running=0):
  """Requests a certain number of VMs. Returns a list of
  ...
  by the plugin. This is done on purpose to avoid overflowing the cloud (possibly
  a non-free one) with misconfigured VMs that don't join the cluster.
  """
...
def running_instances(hostnames=None):
  """Returns all running instances visible with current API credentials, or
...
def scale_down(hosts, valid_hostnames=None):
  """Asks the Cloud to shutdown hosts corresponding to the given hostnames
  by using the API interface. Returns the list of instance IDs shut off
  ...
  # List instances with the "valid" hostnames
  inst = running_instances(valid_hostnames)
  if inst is None or len(inst) == 0:
    logging.warning("No list of instances can be retrieved from API")
    return []
  ...
  for ip in ips:
    found = False
    for i in inst:
      if i.network_ip(network_name=cf["api"]["network_name"]) == ip:
  ...
  logging.debug("Batch hosts: reqd to shutdown=%d | to shutdown matching=%d | total matching=%d" % \
    (len(hosts), len(inst_shutdown), len(inst)))

  # Shuffle the list
  random.shuffle(inst_shutdown)

  # Number of VMs to shutdown to honor the minimum quota of API VMs matching batch hosts
  max_vms_to_shutdown = len(inst)-cf['quota']['min_vms']  # inst --> known *both* by the batch system and API
  ...
  if max_vms_to_shutdown <= 0:
    ...
    for i in inst_shutdown:
      ...
      ipv4 = i.network_ip(network_name=cf["api"]["network_name"])
      success = False
      if int(cf['debug']['dry_run_shutdown_vms']) == 0:
        try:
          i.terminate(token_id=api.keystone.token_id)
          list_shutdown_ok.append(i.id)
          owned_instances.remove(i.id)
          logging.debug("Shutdown via API of %s succeeded" % ipv4)
          success = True
        except Exception, e:
          logging.error("Shutdown via API failed for %s" % ipv4)
      else:
        # Dry run
        logging.debug("Not shutting down %s via API: dry run" % ipv4);
        success = True
...
def image(image_id):
  """Returns an image object containing the image corresponding to a
  certain image ID, or None if not found or problems occurred."""
...
def renew_token(st):
  ...
  api.connect(keystone_url=cf['api']['server_url'],
              username=cf['api']['username'],
              password=cf['api']['password'],
              tenantName=cf['api']['tenant_name'])

  return {
    'action': 'renew_token',
    'when': time.time() + cf['api']['renew_token_every_s']
    }
...
def main(argv):
  ...
  global api, img, flavor, keypair, network, user_data, BatchPlugin, state_file
  ...
  img = image(cf['api']['image_id'])
  if img is None:
    logging.error("Cannot find image \"%s\"", cf['api']['image_id'])
  else:
    logging.debug("Image \"%s\" found" % cf['api']['image_id'])

  # Initialize flavor
  flavor = api.nova.get_flavor(token_id=api.keystone.token_id, tenant_id=api.keystone.tenant_id(cf["api"]["tenant_name"]), flavor_name=cf["api"]["flavor"])
  if flavor is None:
    logging.error("Cannot find flavor \"%s\"", cf['api']['flavor'])
  else:
    logging.debug("Flavor \"%s\" found" % cf['api']['flavor'])

  # Initialize keypair
  keypair = api.nova.get_keypair(token_id=api.keystone.token_id, tenant_id=api.keystone.tenant_id(cf["api"]["tenant_name"]), keypair_name=cf["api"]["key_name"])
  if keypair is None:
    logging.error("Cannot find keypair \"%s\"", cf['api']['key_name'])
  else:
    logging.debug("Keypair \"%s\" found" % cf['api']['key_name'])

  # Initialize network
  network = api.neutron.get_network(token_id=api.keystone.token_id, network_name=cf["api"]["network_name"])
  if network is None:
    logging.error("Cannot find network \"%s\"", cf['api']['network_name'])
  else:
    logging.debug("Network \"%s\" found" % cf['api']['network_name'])

  # Un-base64 user-data
  try:
    user_data = base64.b64decode(cf['api']['user_data_b64'])
  except TypeError:
  ...
  while do_main_loop == True:
    ...
    for evt in internal_state['event_queue'][:]:
      ...
      if evt['when'] <= check_time:
        ...
        elif evt['action'] == 'renew_token':
          r = renew_token(internal_state, *p)
